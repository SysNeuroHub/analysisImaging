function TexVol_T2 = Stereo2T2(imageStereo, bregma, usFactor, Vusinfo, tform3, surfDepth, mrangle)
...
sizeVus = Vusinfo.ImageSize;
   ...
   if verbose
        fig = figure;
        subplot(2,3,1); imagesc(imageStereo); hold on; plot(bregma(2), bregma(1),'ro'); axis equal tight;
    ...
    %% texture mapping from 2D to 3D in Allen CCF (Kim) space
     TexVol_CCF = beforeniftiwrite(paintSurfaceToVolume(surfDepth, img_reg, sizeVus)); %CORRECT
