...
localDir = 'C:\svdinput'; %originally /mnt/data/svdinput/
rawDataDir = 'C:\ThorCamData'; 

%% Open and timestamp log file
log_path = fullfile(localDir, 'svd_pipeline/svd_pipeline_logs/');
...
pipeline_files = dir(rawDataDir);
pipeline_files = pipeline_files(~pipeline_files.isdir);
pipeline_filenames = fullfile({pipeline_files.folder}, {pipeline_files.name});
...
for curr_pipeline = 1:length(pipeline_filenames)
    ...
    try 
        ...
        % Load options
        load ops.mat; %where should I put this? what is in it?
        %ops.remoteDataSource
        %ops.localDataDest
        %ops.verbose
