...
expt.expDate = '2021-01-09_2';%'2020-11-05_1';%
expt.expNum = 1;
bklightCtrl = 0;
...
ABSMAPS_u = zeros(Uy,Ux,p.nrepeats, p.nstim);
ABSMAPS_d = zeros(Uy,Ux,p.nrepeats, p.nstim);
for iTr = 1:p.nrepeats
    
    thisRepeat = (1:p.nstim) + p.nstim*(iTr-1);
    ...
    theseEvents = find(stimSequence.seq == iStim);
       ...
       thisEventV = squeeze(periEventV(theseEvents(iTr),:,:));%omit the 1st dimension
        ...
        %Tensor = Tensor - svdFrameReconstruct(U,mean(mean(avgPeriEventV,1),3)'); %subtract gnd avg
        Tensor = Tensor - svdFrameReconstruct(U,mean(mean(periEventV(thisRepeat,:,:),1),3)'); %subtract avg of this repeat 8/1/21
        ...
        [a, AbsMaps_u] = FourierMaps(Tensor, Fs, tf_corrected*1.1);
        [a, AbsMaps_d] = FourierMaps(Tensor, Fs, tf_corrected*0.9);
        ABSMAPS_u(:,:,iTr, iStim) = AbsMaps_u;
        ABSMAPS_d(:,:,iTr, iStim) = AbsMaps_d;

    end
end


%% VISUALIZE AMP/PHASE OF SINGLE TRIALS
...
theseRepeats = 1:p.nrepeats;
randidx = [];
for ii = 1:4
    
    if strcmp(expt.expDate, '2020-11-05_1')
        theseRepeats = [1 2 3 4 ];
        if ii==1
            theseRepeats = [1 2 3];
        end
    end
    randidx_c = repmat(theseRepeats,1,n_boot);
    randidx(ii,:) = randidx_c(randi(n_boot,1,n_boot));
end
