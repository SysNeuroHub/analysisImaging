...
expt.subject = 'TIGRE2GCaMP6s_315';
expt.expDate = '2020-10-16_1';
expt.expNum = 1;
...
load(fullfile(resultSaveDir,[figname 'signMap']), 'signMap_median','xMapm_vf','yMapm_vf','resizeS');
signMap_median = imresize(signMap_median, 1/resizeS);
xMapm_vf = imresize(xMapm_vf, 1/resizeS);
yMapm_vf = imresize(yMapm_vf, 1/resizeS);

movieSuffix = {'blue','purple'};
crange = [1 80];
mimg_norm = [];
for isuffix = 1:length(movieSuffix)
   ...
   [~, ~, ~, mimg, mask] = quickLoadUVt(expPath, nSV, saveVpath, params);
    ...
    ax(1)=subplot(131);imagesc(xMapm_vf,'alphadata',mimg_norm(:,:,isuffix));axis equal tight;
    caxis(prctile(xMapm_vf(:),[1 99]));mcolorbar(gca,.5);title('pref x');
    axis off
    
    ax(2)=subplot(132);imagesc(yMapm_vf,'alphadata',mimg_norm(:,:,isuffix));axis equal tight;
    caxis(prctile(yMapm_vf(:),[1 99]));mcolorbar(gca,.5);title('pref y');
    axis off
