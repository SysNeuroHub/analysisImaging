...
roi = [size(tavgResp,1)/2-size(tavgResp,1)/4 size(tavgResp,2)/2-size(tavgResp,2)/4 size(tavgResp,1)/2 size(tavgResp,2)/2];

nRows            = 1;%ceil(sqrt(p.nstim));
nCols = 5;%p.nstim-1;%ceil(p.nstim/nRows);
figure('position',[0 0 1900 600]);
panel = [];
for istim = 2:6;%p.nstim-1
    panel(istim) = subplot(nRows,nCols,istim-1);
...
g.YLabel.String='dF [pixel value]';
screen2png(figname);
close;

tsavgResp = squeeze(mean(mean(tavgResp(roi(1):roi(1)+roi(3), roi(2):roi(2)+roi(4),:))));

mV = [];
for istim = 2:6
mV(istim) = str2num(stimSequence.labels{istim}(25:28));
end
figure;
plot(mV(2:6), tsavgResp(2:6),'-o');
xlabel('input voltage to laser [mV]');
ylabel('avg pixel values in the center');
axis padded;
screen2png(['avg_' figname]);
close;
