function [protocol, successflag] = ProtocolLoad_wf( animal, iseries, iexp, screenflag, DIRS_data)
% loads protocol data structure
%
...
% 2020-1-16 fixed filename to load


if nargin < 5   
    % 2015-09-17 MC added the next 3 lines to avoid crash if DIRS doesn't exist 
    % 2016-05-12 DIRS was made as global variable before these lines, so
    % the condition was never satisfied? made DIRS global after this check
    if ~exist('DIRS','var')
        SetDefaultDirs;
    end
    global DIRS
...
if isempty(iseries), error('No series was specified'); end
if isempty(iexp   ), error('No experiment was specified'); end

%ProtocolFileName = fullfile(DIRS_data,animal,iseries,int2str(iexp), 'Protocol.mat');
thisDate = iseries(1:10);
thisSeries = str2num(iseries(12:end));
ProtocolFileName = dat.expFilePath(animal, thisDate, thisSeries, iexp, 'parameters','master');

%commented out 16/1/20
% if exist(fullfile(DIRS.oldData, animal), 'dir') % point to zserver/Data/trodes if the animal folder exists
%     if isnumeric(iseries)
%         iseries = int2str(iseries);
%     end
%     ProtocolFileName = fullfile(DIRS.oldData,animal,iseries,int2str(iexp), 'Protocol.mat');
% else % point from zubjects/Subjects for new data
%     iseries = int2str(iseries);
%     iseries = [iseries(1:4), '-', iseries(5:6), '-', iseries(7:8)];
%    ProtocolFileName = fullfile(DIRS_data,animal,iseries,int2str(iexp), 'Protocol.mat');
% end
...
if exist( ProtocolFileName, 'file' )
    ...
    if isfield(foo.parameters,'Protocol')
        % one of the new protocols, made by mpep...
        protocol = foo.parameters.Protocol;%foo.protocol; %3/6/20
...
elseif exist( ProtocolFileName, 'file' )
...
%         end
    ...
    fullpfilename1 = fullfile(DIRS_data,animal,int2str(iseries),int2str(iexp),pfilename);
    fullpfilename2 = fullfile(DIRS_data,animal,int2str(iseries),pfilename);
...
switch protocol.xfile
   ...
   case 'vXYCMSeqGratVer2.x'
    ...
    case 'stimColorBar_DS.x'
        protocol.blankpars = 12;
        protocol.pfiledurs = protocol.pars(1,:)/10;
        % CORRECT FOR BUG THAT OCCURRED ONLY IN ONE EXPERIMENT
        if strcmp(protocol.animal,'M140430_SD')
            disp('ProtocolLoad APPLYING A HACK AND ADDING 10 TO START AND END TIMES');
            protocol.pars([2 3],:) = protocol.pars([2 3],:) + 10;
        end
...
%         protocol.pfilefreqs = 1./(protocol.pars(1,:)/10); % frequency is 1/duration
    ...
    case {'stimKalatskyCombo.x'} %added by AP Sept 2014 
        protocol.blankpars = [12 13 14]; 
        protocol.pfiledurs = protocol.pars(1,:)/10;
%         protocol.pfilefreqs = 1./(protocol.pars(1,:)/10); % frequency is 1/duration
        protocol.ncycles = [protocol.pars(1,:).*protocol.pars(6,:); protocol.pars(1,:).*protocol.pars(7,:)]/1000; 
...
%          protocol.pfilefreqs = [] % frequency is 1/duration
    ...
    case {'vmovie3sequentialGrating.x'}
        ...
        protocol.pfiledurs = protocol.pars(1,:)/10; %24/7/20
        protocol.blankpars = 15; % this assumes that if c1 is zero it is a blank... %3/6/20
...
%         protocol.pfilefreqs = (protocol.pars(14,:)~=0).*(protocol.pars(15,:)); % frequency of optical stimulus
    ...
    case {'stimTwoWaveOutputs.x'} %added by MW April 2015
         protocol.blankpars  = [2:3]; % wave amplitude
         protocol.pfilefreqs = 1./(protocol.pars(4,:)/10); % 
...
if isstr(screenflag) & ((strcmpi(animal,'catz') & str2num(animal(end-2:end)) > 12) | ~strcmp(lower(animal),'catz')) ...
    ...
    if ~isempty(nif) & ~isempty(protocol.pfilefreqs)
        [motherdir,datadir] = fileparts(DIRS_data);
