...
function f = pixelTuningCurveViewer(allFrames, cLabels, timePoints, alphaData)
...
if isempty(cLabels) || nargin < 2
    cLabels = 1:nConditions;
end
if isempty(timePoints) || nargin < 3
...
set(f, 'KeyPressFcn', @(f,k)tcViewerCallback(f, k, allFrames, cLabels, timePoints, alphaData));
set(f, 'WindowScrollWheelFcn', @(f,k)tcViewerCallbackWheel(f,k, allFrames, cLabels, timePoints, alphaData));

showTC(allFrames, cLabels, timePoints, thisPixel, thisTimePoint, thisCond, cax, alphaData);

function tcViewerCallbackWheel(f, keydata, allFrames, cLabels, timePoints, alphaData)
...
function tcViewerCallback(f, keydata, allFrames, cLabels, timePoints, alphaData)
...
function tcViewerCallbackClick(f, keydata, allFrames, cLabels, timePoints, alphaData)
...
function showTC(allFrames, cLabels, timePoints, thisPixel, thisTimePoint, thisCond, cax, alphaData)
...
if ~isempty(alphaData)
    q = imagesc(allFrames(:,:,thisTimePoint,thisCond), 'alphadata', alphaData,'alphadatamapping','scaled');
    set(q, 'HitTest', 'off');
else
...
thisAx = subplot(1,4,3); 
 %colors = [0 0 1; 0 0 0.8; 0 0 0.5; 0 0 0;  0.5 0 0; 0.8 0 0; 1 0 0];
colors = copper(nConditions+1); colors = colors(2:end, [3 2 1]);
for f = 1:nConditions
...
yl = ylim();
q = plot([timePoints(thisTimePoint) timePoints(thisTimePoint)], [cax(1) cax(2)], 'r'); set(q, 'HitTest', 'off');
q = plot([0 0], [cax(1) cax(2)], 'r--'); set(q, 'HitTest', 'off');
hold off;
ylim(cax);
...
q = plot(cLabels, tc, 'ro'); hold on; set(q, 'HitTest', 'off');
q = plot(cLabels(thisCond), tc(thisCond), 'r*'); set(q, 'HitTest', 'off');
hold off;
ylim(cax);
midC = (max(cLabels)+min(cLabels))/2; cRange = [max(cLabels)-min(cLabels)]*1.1;
