function [numExps, nFrPerExp, allT, existExps, alignmentWorked] ...
    = determineOEphysAlignments(ops, nFrInV)
...
%       thisDate: str
%       verbose: logical
%       rigName: str. Determines which signal is pulled out of Timeline for the timestamps, see function getStrobeTimes
%       frameMod: 1x2. Specifies which frames to use according to: mod(frameNums, frameMod(1))==frameMod(2);
%       expRefs: cell array of strings, optional. Which expRefs to use.
%       inclExpList: array of integers, optional. Which expNums to use, just an alternative to specifying expRefs.
%   nFrInV: number of frames in V of SVD
%
% Outputs:
%   numExps: 1x1, the number of experiments that were found and aligned (0 if alignment fails)
%   nFrPerExp: 1 x numExps, the number of frames in each experiment
%   allT: 1 x numExps cell array, each cell contains a 1 x nFrPerExp(n) array of the timestamps(in Timeline coordinates) of each frame
...
%TEMP
rootFolder = fileparts(fileparts(expFilePathOEphys(ops.mouseName, ops.thisDate,1)));
d = dir(rootFolder);
numExps = length(d)-2;
if numExps<1
    if ops.verbose
    ...
    if isfield(ops, 'expRefs') && ~isempty(ops.expRefs)
        ...
        for e = 1:length(expNums)
            expRefs{e} = dat.constructExpRef(ops.mouseName, ops.thisDate, ...
                expNums(e));
        end
    end
    ...
    for e = 1:length(expRefs)
        ...
        %TEMP. FIX TO DEFINE DIR FROM EXPREFS
        jsonFile = expFilePathOEphys(ops.mouseName, ops.thisDate, expNums(e));
        if exist(jsonFile, 'file')
            %load(timelinePath)
            %strobeTimes = getStrobeTimes(Timeline, ops.rigName);
            strobeTimes = getStrobeTimesOE(jsonFile, ops.rigName);
