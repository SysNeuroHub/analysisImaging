function [FTOSignalValid, stimulusSignalValid]=filterFTObyStimulus(syncDataOut, showFigs)
...
captureRising (find(captureEvents> 0.5))=1;
captureFalling(find(captureEvents<-0.5))=1;
...
for I=1:sum(captureRising),  %Do once for each capture active rising edge (which should equal the number of falling edges)
    ...
    foundStimulusFallingIdx=min(find((stimulusFallingInvalidIdx>captureRisingIdx) & (stimulusFallingInvalidIdx<captureFallingIdx)));  %Find the first falling edge that is later than the rising edge and earlier than the falling edge.
