...
%10/7/20 revised stimulus timing detection with grabStimTimes2p.m
% confirmed to work:
% expt.subject = 'Newton';
% expt.expDate = '2020-07-09_1';
% expt.expNum = 5;
% ori8full, detect each stimulus onset
% TS saves Screen, syncSquare, stimScreen
% syncSquare is in "Steady" mode
...
% ori8full, detect each stimulus onset
% TS saves Screen, syncSquare, stimScreen
% syncSquare is in "Flicker" mode


%for protocolLoad_wf ... now compatible w 2p as far as data is uploaded to
%Market server
...
cd('C:\Users\Analysis\Documents\MATLAB\analysis\ThorScanAnalysis');
...
%p = ProtocolLoad_wf(expt.subject,expt.expDate,expt.expNum);%10/7/20 now this works!
...
thPhd = 0.5;%'userInput';%
expt = grabStimTimes2p(expt, false, TSDir, thPhd, .9*ISI); %10/7/20
% [ expt ] = grabStimTimes( expt, false, TSDir, thPhd, .9*ISI);
% expt.stimTimes(expt.expNum).onset(consecutive stimulus Number)
% expt.stimTimes(expt.expNum).offset(consecutive stimulus Number)
...
savefig(gcf, fullfile(resultSaveDir, ['ROI']));
