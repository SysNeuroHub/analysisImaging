...
p = ProtocolLoad_wf(expt.subject,expt.expDate,expt.expNum);
...
%% detect 2p frame time in thorsync time
[ frameTimes, TIdata ] = grabFrameTimes( expt, expt.expNum, 'all', TSDir); %[1 x 2pframes]
...
expt = grabStimTimes2p(expt, true, TSDir,1); %10/7/20
% ISI = min(p.pfiledurs);
...
expt = checkStimFrames(expt, size(stim_screen,3));
