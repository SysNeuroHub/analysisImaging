...
for v = 1:length(ops.vids)
    ...
    dataSummary = loadRawToDat_ROI(loadDatOps);
    
    disp([num2str(dataSummary.nFrames) ' frames saved to ' loadDatOps.datPath]);
...
if ops.doRegistration
    ...
    targetFrame = determineTargetFrameSingle(datPath, imageSize, nFr, regOps);
...
for v = 1:length(ops.vids)
    ...
    t=toc;
    disp(['Processing SVD took ' num2str(t) '[s]']);
    ...
    saveSVDns(saveOps, U, V, results(v)); 
...
%% delete original data. Disabled 3/11/21
% % if ~isequal(ops.vids(1).fileBase, destFolder)
% %     try
% %         for ee = 1:length(ops.inclExpList)
...
if length(dir(ops.localSavePath))==2 % if the folder is empty
    ...
    %rmdir(ops.localSavePath);%for this to work, directory must be empty
    delete(fullfile(ops.localSavePath,'*'));
