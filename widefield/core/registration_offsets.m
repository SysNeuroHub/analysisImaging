function [dv, corr, regdata] = registration_offsets(data, ops, refImg, removeMean)
% [dv, corr, regdata] = registration_offsets(data, ops, refImg, removeMean)
% computes shifts (dx and dy) to align to refImg at subpixel resolution
%
% Inputs:
%   data: Y x X x nFrames
%   ops must contain:
%       'subPixel' or 'SubPixel': subpixel factor
%       'registrationUpsample': factor to upsample
%       'phaseCorrelation' or 'PhaseCorrelation'
%       'smooth_time_space': temporal & spatial smoothing factors [time space]
%   refImg: target image to compute the offset
%   removeMean: if true, output "dv" is after subtracting its mean across frames [true/false]
%
% Outputs:
%   dv: shift size with respect to refImg in pixels
%   corr: correlation coefficient between refImage and registred frame
%   regdata: Y x X x nFrames after registration
%
% called by alignToTarget.m
% originally written for suite2p
%
% TODO:
% use ops.roi if defined?
...
ops.smooth_time_space = getOr(ops, 'smooth_time_space',[15 1]); %20/10/20
...
if ~isempty(ops.smooth_time_space)
    data = smooth_movie(data, ops); %20/10/20
end
