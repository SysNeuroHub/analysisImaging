function [Ub, Vcorr, tb, mimgB] = quickHemoCorrect_binTest(expPath, savePath, nSV, ...
...
Up = cat(1, Up, nan(8,size(Up,2),size(Up,3)));%HACK
Ub = cat(1, Ub, nan(8,size(Ub,2),size(Ub,3)));%HACK
...
for bb = 1:5
    ...
    screen2png(fullfile(savePath,['HemoCorrect_sanityCheckFigs_' num2str(round(hemoFreq)) ' Hz_bin' num2str(thisBin)]));%19/11/20
    close(fig);
    
    % % Vcorrfilt = detrendAndFilt(Vcorr, Fs); %5/1/18
    % % svdViewer(Ub, DSb.Sv, Vcorrfilt, Fs) %blue  after filtering and detrending
    
    writeUVtoNPY([], Vcorr, [], fullfile(savePath, ['svdTemporalComponents_corr_bin' num2str(thisBin)]));
...
function binned = binTensor(original, binSize)
binned = nan(size(original,1)/binSize, size(original,2)/binSize, size(original,3));
for tt = 1:size(original,3)
    binned(:,:,tt) = binImage(original(:,:,tt), binSize);
end
end