function [Vout, T, ScaleFactor] = HemoCorrectLocal(U, V, Vaux, fS, FreqRange, pixSpace)
% [Vout, T, ScaleFactor] = HemoCorrectLocal(U, V, Vaux, fS, FreqRange, pixSpace)
% 
% Does local hemodynamic correction for widefield imaging, in SVD space.
%
% Inputs:
% U and V: an SVD representation of is the neural signal you are trying to correct
%
% Vaux: the other non-neural signals that you are using to measure hemodynmaics 
...
%
% FreqRange: the heartbeat range, in which the correction factors are estimated. 
% Default 9 to 13.
%
% pixSpace: the spacing between the pixel subgrid on which to compute the gain
% factors. Larger means faster but possibly less accurate. (Default 3)
%
% Outputs: 
% Vout: corrected signal
% T: transformation matrix that predicts V from Vaux
...
ScaleFactor = reshape(ScaleFactor, size(pixY))';
% plot it for sanitycheck fig
if showFigs
    figure;
