...
% movieWithTracesSVD(U, V, t, traces, movieSaveFilePath[, auxVid])
% shows a movie with traces at specified ROI
% Inputs:
%   U: yPix x xPix x nSV
%   V: nSV x nTimePoints
%   t: 1 x nTimePoints, the labels of V
%   traces: a struct array with three fields:
%       t: 1 x nTimePoints
%       v: 1 x nTimePoints
%       name: a string
%   auxVid: a struct array with fields:
...
%       name is a string
%   movieSaveFilePath: the movie name with suffix (.avi), needed to record
%
% Usage:
% - 'p' plays/pauses
...
%     ax = axes();
    ...
    ud.ImageHandle = myIm;
    caxis(cax);
    colormap(bone);%(colormap_blueblackred);
    %colormap parula
    axis equal tight;
    axis image
    colorbar    
%     set(myIm, 'HitTest', 'off');
    ...
    for tInd = 1:nSP-1
        ...
        r = plot([currTime+windowSize/2-1 currTime+windowSize/2], [yl(1) yl(1)],'color','k');
        ud.traceScaleBars(tInd) = r;
    ...
    r = plot([currTime+4 currTime+5], [yl(1) yl(1)], 'k');    
    ud.traceScaleBars(nSP) = r;
...
if ud.playing
    ...
    for n = 1:nSP
        ...
        set(ud.traceScaleBars(n), 'XData', [currTime+windowSize/2-1 currTime+windowSize/2], ...
            'YData',[cax(1) cax(1)]); %24/6/20
...
%         line([currTime+windowSize/2-1 currTime+windowSize/2], ...
%             [cax(1) cax(1)],'color','k','linewidth',2);
