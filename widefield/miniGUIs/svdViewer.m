...
% svdViewer(U, Sv, V, Fs[, totalVariance])
% shows U and V interactively. 
%
% Usage:
...
myData.pixelY = pY;
myData.pixelX = pX;
myData.pixelMode = true;%false;%11/6/20
...
f = get(s, 'Parent');
myData = get(f, 'UserData');
myData.pixelY = round(e.IntersectionPoint(2)); %1
myData.pixelX = round(e.IntersectionPoint(1)); %2
...
axis equal tight; set(ax, 'XTick', [], 'YTick', []);
hold on; 
if myData.pixelMode
    plot(pX, pY, 'ro'); set(q, 'HitTest', 'off');
end
